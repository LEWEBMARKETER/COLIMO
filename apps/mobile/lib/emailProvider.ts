...
// la place du MockEmailProvider par défaut — cf.
// packages/shared/src/communication/providers. N'appeler qu'une seule fois,
// au démarrage de l'app web (apps/mobile/app/_layout.tsx), même schéma que
// pushProvider.ts. Chaque envoi continue d'être journalisé dans
// `notifications` exactement comme avant ; ce fournisseur ajoute en plus un
// vrai envoi via Resend.
...
const ResendEmailProvider: EmailProvider = {
  ...
  async envoyer({ destinataire, sujet, contenu }): Promise<ResultatEnvoi> {
    try {
      ...
      const reponse = await fetch("/api/email/envoyer", {
        method: "POST",
